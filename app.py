...
try:
    ...
    st.error("Model file 'forecasting_ev_model.pkl' not found. Please make sure it exists in the same directory.")
    st.stop()
...
# Image (safe loading)
if os.path.exists("ev-car-factory.jpg"):
    st.image("ev-car-factory.jpg", use_container_width=True)
else:
    st.warning("Image file 'ev-car-factory.jpg' not found. Please add it to the project folder.")
...
def load_data():
    try:
        ...
        st.error("Dataset 'preprocessed_ev_data.csv' not found.")
        st.stop()
...
st.markdown("Prepared for the AICTE Internship Cycle 2 by Mayank")
